...
export async function generateDailyWords(): Promise<Word[]> {
  ...
  if (!ai) {
    console.warn("GEMINI_API_KEY is not configured. Using fallback words.");
    return FALLBACK_WORDS.map((word) => ({ ...word, date_shown: today }));
  }
        ...
        responseSchema: {
          ...
          items: {
            ...
            properties: {
              ...
              pronunciation: {
                type: Type.STRING,
                description: "A phonetic guide like 'pro-NUN-see-AY-shun'",
              },
            },
...
export async function generateRandomWord(): Promise<Word> {
  ...
  if (!ai) {
    console.warn(
      "GEMINI_API_KEY is not configured. Using fallback random word.",
    );
...
const FALLBACK_WORDS: Word[] = [
  ...
  },
  {
    ...
    example:
      "The diplomat gave an eloquent speech that moved the entire assembly.",
    part_of_speech: "Adjective",
  ...
  },
  {
    ...
    example:
      "The community showed its resilient spirit by rebuilding immediately after the storm.",
    part_of_speech: "Adjective",
  ...
  },
  {
    ...
    example:
      "The CEO was known for her sagacious decision-making during the financial crisis.",
    part_of_speech: "Adjective",
